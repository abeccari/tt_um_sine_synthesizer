...
4. **Outputs** —
   ...
   - `uio[1]` — **PDM_Q**, the same for the cosine (90° out of phase with PDM_I) — an I/Q pair.
...
4. **Monitor** —
   ...
   - Scope `uo[7]` (PDM_I) and `uio[1]` (PDM_Q): fast 1-bit streams whose pulse density follows the sine and cosine.
...
- **Quadrature (I/Q):** filter `uio[1]` (PDM_Q) the same way for a second channel 90° out of phase — useful for demodulation experiments.
